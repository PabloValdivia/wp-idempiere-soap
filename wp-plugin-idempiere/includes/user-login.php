...
$verifurl=site_url()."/index.php/user-verification/";
...
require_once('general-setting.php');
...

  {
...

    {
      ...
      {
        $post_string='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:_0="http://idempiere.org/ADInterface/1_0"><soapenv:Header/> <soapenv:Body> <_0:queryData><_0:ModelCRUDRequest><_0:ModelCRUD><_0:serviceType>wp-queryUsers</_0:serviceType><_0:DataRow> <!--Zero or more repetitions:--><_0:field column="EMail"><_0:val>'. $name .'</_0:val></_0:field> </_0:DataRow></_0:ModelCRUD>'.$login_request.'</_0:ModelCRUDRequest></_0:queryData></soapenv:Body></soapenv:Envelope>';
      }
      else
      {
 ...
 /*     
      $getid=$vals[7]['value'];             //AD_User_ID
	  $getemail= $vals[19]['value'];        //email
	  $getuser_name=$vals[34]['value'];     //Value
      $getemail_verify=$vals[28]['value'];
      $getname=$vals[10]['value'];              //Name
      $getpass=$vals[13]['value'];              //password
      $getverify_status=$vals[37]['value'];     //isLocked 
 */     
          ...
          {  
           ...
           $_SESSION['admin_user_id']=$getid;
          ...
          else
            {
              ...
              $verifurl=site_url()."/index.php/verification?username=".$getuser_name;
              $welcome_string='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:_0="http://idempiere.org/ADInterface/1_0">
                 <soapenv:Header/>
                 <soapenv:Body>
                    <_0:createData>
                       <_0:ModelCRUDRequest>
                             ...
                             <_0:serviceType>wp-userRQ</_0:serviceType>
                              <_0:DataRow>
                                ...
                                <_0:field column="Summary">
                                   <_0:val>Dear'. $getname .',\n Thank you for registering with us, the innovative way to manage your properties and shorten the communication gap between Landlords and Brokers
              please follow verification code '. $getemail_verify .' and link '. $verifurl .' to verify your account email</_0:val>
                                </_0:field>
             ...
             /* echo "<pre>";
              ...
              if($welcome_output!='')
              {
                echo "<p class='success'>User Email not verified.Please check your Email and complete the verification process.</p>";
              }  
  ...
  {
    $loginurl=site_url()."/index.php/user-login/";
